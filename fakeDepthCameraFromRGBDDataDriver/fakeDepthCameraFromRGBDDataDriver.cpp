...
yarp::dev::fakeDepthCameraFromRGBDDataDriver::fakeDepthCameraFromRGBDDataDriver() : rgb_h(480),
                                                                                    rgb_w(640),
                                                                                    dep_h(480),
                                                                                    dep_w(640),
                                                                                    rgb_Vfov(36),
...
bool yarp::dev::fakeDepthCameraFromRGBDDataDriver::open(yarp::os::Searchable &config)
{
    ...
    std::string startFrame = "00000000";
...
bool yarp::dev::fakeDepthCameraFromRGBDDataDriver::getDepthImage(yarp::sig::ImageOf<yarp::sig::PixelFloat> &depthImage, Stamp *timeStamp)
{
    ...
    yarp::sig::ImageOf<yarp::sig::PixelMono16> depth_image;
    ...
    // fileValid = yarp::sig::file::read(depthImage,imageTest,yarp::sig::file::FORMAT_NUMERIC);


    // FILE *fp = fopen(imageTest.c_str(), "rb");
    // if (fp == nullptr) {
    //     return false;
    // }
    ...
    // size_t pad = depthImage.getRowSize();
    // size_t bytes_to_read_per_row = w* depthImage.getPixelSize();
    // unsigned char* dst = depthImage.getRawImage();
    // size_t num = 0;
    ...
    // return (num > 0);
    // float *rawImage;
    // unsigned char * srcRawImage = depthImage.getRawImage();
    // rawImage = reinterpret_cast<float *>(depthImage.getRawImage());
    // for (int i = 0; i < depthImage.width() * depthImage.height(); i++)
    // {
    //     rawImage[i] = srcRawImage[i] * factor;
